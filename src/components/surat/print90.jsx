...
function Print() {
  ...
  const headers = [
    { label: "Nomor DO", key: "sppg" },
    { label: "Tanggal DO", key: "tanggalSPPB" },
    { label: "No SK", key: "sk" },
    { label: "Tanggal SK", key: "tanggalSK" },
    { label: "Nama Pengambil", key: "pengambil" },
    { label: "Alamat", key: "alamat" },
    { label: "Kuanta", key: "kuanta" },
    { label: "Sisa", key: "sisa" },
  ...
  return (
    <>
      <div className="w-40 ml-6 flex flex-col items-start">
        <h2 className="text-xl font-bold mb-4">Nomor SPPG</h2>
        <div className="flex flex-wrap space-y-4">
          {sppgNumbers.map((sppg, index) => (
            <div key={index} className="flex items-center space-x-2">
              <label className="flex text-gray-700 text-sm font-bold w-20">
                SPPG {index + 1}
              </label>
              <input
                type="text"
        ...
        <button
          ...
          className="mt-4 px-4 py-2 bg-red-500 text-white rounded shadow hover:bg-red-600 focus:outline-none"
        >
          -
        </button>
        </div>
      ...
      >
        <div
          ref={printRef}
          className="container flex flex-col mx-0"
          style={{ width: "55vw" }}
        >
          {data.map((item, index) => (
            <div key={index}>
              {index > 0 && (
                <hr className="border border-slate-800 mb-4 rounded-t-2xl border-t-4 border-b-0" />
              )}
      ...
      <div className="flex justify-end mt-8 mr-72">
        ...
        <CSVLink
          data={data}
          headers={headers}
          filename={`data_sppg_${currentDate}.csv`}
          className="text-white bg-green-500 hover:bg-green-600 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none"
        >
          Download CSV
        </CSVLink>
      </div>
